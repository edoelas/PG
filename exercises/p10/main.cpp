...
std::shared_ptr<Scene> loadPBRPistol() {
	...
	auto model = FileLoader::load(App::assetsDir() + "models/Cerberus/Cerberus_LP.FBX");
	auto mat = std::make_shared<PBRMaterial>();

	mat->setBaseColorTexture(loadTexture(App::assetsDir() + "models/Cerberus/Textures/Cerberus_A.png"));
	mat->setMetalnessTexture(loadTexture(App::assetsDir() + "models/Cerberus/Textures/Cerberus_M.png"));
	mat->setNormalMapTexture(loadTexture(App::assetsDir() + "models/Cerberus/Textures/Cerberus_N.png"));
	mat->setRoughnessTexture(loadTexture(App::assetsDir() + "models/Cerberus/Textures/Cerberus_R.png"));
	mat->setAmbientOcTexture(loadTexture(App::assetsDir() + "models/Cerberus/Textures/Cerberus_AO.png"));
...
void MyRender::setup() {
	...
	// Constru�mos el objeto GLMatrices
...
void MyRender::reshape(uint w, uint h) {
	...
	if (h == 0)
		h = 1;
	float ar = (float)w / h;
	// Definimos una c�mara perspectiva con la misma raz�n de aspecto que la
	// ventana
