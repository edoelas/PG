...
class MaRender : public Renderer {
public:
...
bool MaRender::loadGeoTif(const std::string& filename)
{
 ...
 * @param folder 
*/
void MaRender::refreshTilesList(const std::string& folder)
{
	tilesFolder = folder;
	tilesList->clear();
...
void MaRender::reshape(uint w, uint h) {
	glViewport(0, 0, w, h);
...
//	myApp.getWindow().setRenderer(std::make_shared<MaRender>());
//	return myApp.run();
//}
