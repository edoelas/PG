...
void main() {
	...
	// Calculamos la posicion del vertice teselado como una interpolacion bilineal
	vec4 p1 = mix(gl_in[0].gl_Position, gl_in[1].gl_Position, gl_TessCoord.x);
	vec4 p2 = mix(gl_in[3].gl_Position, gl_in[2].gl_Position, gl_TessCoord.x);

	gl_Position = modelviewprojMatrix * mix(p1, p2, gl_TessCoord.y);
	gl_Position.y += texture(texUnitHeightMap, textureCoord).r * 5;
}
