...
void MyRender::setup() {
	...
	vs.push_back(glm::vec3(0.0, 0.0, 1.0));
	vs.push_back(glm::vec3(1.0, 0.0, 1.0));
	vs.push_back(glm::vec3(1.0, 0.0, 0.0));
	...
	DrawArraysInstanced* dc = new DrawArraysInstanced(GL_PATCHES, 0, vs.size(), numQuads * numQuads);
	...
	GLint texUnitcolorScaleHM = program->getUniformLocation("texUnitcolorScaleHM");
	glUniform1i(texUnitcolorScaleHM, 1);
